...
namespace ITRI.Services
{
    public class OrderoutsourceS : BaseService, IOrderoutsourceS
    {
        ...
        public DatatablesVM<Orderoutsource> GetAll(int start, int length,int Id)
        {
            var count = _repository.GetAll().Where(c=>c.OGonNo == Id).Count();
            var data = _repository.GetAll().Where(c => c.OGonNo == Id).Skip(start).Take(length);
